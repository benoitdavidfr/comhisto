...
/*PhpDoc:
name: defeltp.php
title: defeltp.php - définition de chaque version de commune ou d'entité associée comme ensemble d'élts positifs stables dans le temps
doc: |
...
class Version {
  ...
  function __construct(string $cinsee, string $debut, array $version) {
    ...
    $this->elts = new EltSet($version['elts'] ?? '');
    //print_r($version);
  }
...
$histelts = Yaml::parsefile('histelt.yaml');
...
foreach ($histelts['contents'] as $cinsee => &$histo) {
  foreach ($histo as $dv => &$version) {
    if (isset($version['elts'])) {
      $eltSet = new EltSet($version['elts']);
      unset($version['elts']);
      $version['eltsp'] = $eltSet->ajouts($ajouts)->asArray();
    }
  }
}

$histelts['title'] = "Historique des codes Insee augmenté de la définition de chaque version comme ensemble d'éléments positifs";
$histelts['@id'] = "http://id.georef.eu/comhisto/zones/histeltp";
