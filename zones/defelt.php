...
if (php_sapi_name() <> 'cli') {
  ...
  if (!isset($_GET['action'])) {
    echo "</pre><a href='?action=affiche une version intermédiaire'>showF1</a><br>\n";
...
class Histo {
  ...
  /*function setDateElt(): void { // affecte la date de l'élt
    $this->dateElt = array_keys($this->versions)[0];
    foreach ($this->versions as $dv => $version) {
      foreach ($version->evts() as $evtVerb => $evtObjects) {
        switch ($evtVerb) {
          ...
          case 'crééeAPartirDe': return;
          
          case 'fusionneDans': return;
          case 'absorbe': return;
          case 'crééeCommeSimpleParScissionDe': break;
          case 'crééCommeArrondissementMunicipalParScissionDe': return;

          case 'seScindePourCréer': {
            //echo '<b>',Yaml::dump([$this->cinsee => $this->asArray()], 3, 2),"</b>";
            $this->dateElt = $dv;
            //echo Yaml::dump([$this->cinsee => $dv]);
            break;
          }
          
          case 'sAssocieA': return;
          case 'prendPourAssociées': return;
          case 'devientDéléguéeDe': return;
          case 'prendPourDéléguées': return;
          
          default: throw new Exception("evtVerb=$evtVerb");
        }
      }
    }
  }*/
  ...
  function deduitElts(EltSet $elts): EltSet {
    ...
    foreach ($this->evts as $evtVerb => $evtObjects) {
      switch ($evtVerb) {
        ...
        case 'seDissoutDans': { $elts->empty(); break; }
